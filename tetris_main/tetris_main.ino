...
int highscoreAddress = 0; // Address to store the highscore in EEPROM
int highscore = 0; // Variable to store the highscore
...
public:
  ...
  void showOnDeck() {
    ...
    tft.setCursor(DECK_OFFSET_X, DECK_OFFSET_Y + 100);
    tft.setTextSize(3);
    tft.setTextColor(WHITE);
    tft.println("HIGH");

    tft.setCursor(DECK_OFFSET_X, DECK_OFFSET_Y + 150);
    tft.setTextSize(3);
    tft.setTextColor(WHITE);
...
public:
  ...
  void drawGameOverScreen() {
    ...
    EEPROM.write(highscoreAddress, highscore); // Save the new highscore to EEPROM
  }
    tft.fillScreen(BLACK);
    tft.setCursor(screenWidth / 5, screenHeight / 5);
    tft.setTextSize(6);
    tft.setTextColor(WHITE);
    tft.println("GAME OVER");

    tft.setCursor(screenWidth / 5, screenHeight / 2.8 + 50);
    tft.setTextSize(3);
    tft.setTextColor(WHITE);
    tft.print("Your score: ");
    tft.print(score);

    tft.setCursor(screenWidth / 5, screenHeight / 2.8 + 100);
    tft.setTextSize(3);
    tft.setTextColor(WHITE);
    tft.print("Highscore: ");
    tft.print(highscore);
...
void rotate() { //Orientation change procedure
  int JoyStickY = map(analogRead(JOYSTICK_Y_PIN), 0, 1023, 10000, -10000);
...
void setup() {
  ...
  highscore = EEPROM.read(highscoreAddress); // Load highscore from EEPROM
...
// Modified to look awesom
void drawStartScreen() {
  tft.fillScreen(WHITE);                                                // set the background color
  //tft.drawRect(screenWidth / 3, screenHeight / 2.5, 200, 110, WHITE);  // draw a rectangle
  tft.setTextSize(6);
  tft.setCursor(screenWidth / 3.5, screenHeight / 5);
  tft.setTextColor(BLACK);
  tft.print("TETRIS");
  tft.setTextSize(2);
  tft.setCursor(screenWidth / 7, screenHeight - (screenHeight / 3));
  tft.print("Push the joystick up to start");

  // Draw T-shape
  int o_start[4][2] = { { 0 + MIDDLE_OFFSET, 0 }, { 1 + MIDDLE_OFFSET, 0 }, { 2 + MIDDLE_OFFSET, 0 }, { 1 + MIDDLE_OFFSET, 1 } };
  for (int i = 0; i < 4; i++) {
      tft.fillRect(screenWidth / 10 + (o_start[i][0] * BLOCK_SIZE), screenHeight / 2.3 + (o_start[i][1] * BLOCK_SIZE), BLOCK_SIZE - 1, BLOCK_SIZE - 1, RED); //RED
    }

  // Draw L-shape
  int l_start[4][2] = { { 0 + MIDDLE_OFFSET, 0 }, { 1 + MIDDLE_OFFSET, 0 }, { 2 + MIDDLE_OFFSET, 0 }, { 2 + MIDDLE_OFFSET, 1 } };
  for (int i = 0; i < 4; i++) {
      tft.fillRect(screenWidth / 4 + (l_start[i][0] * BLOCK_SIZE), screenHeight / 2.3 + (l_start[i][1] * BLOCK_SIZE), BLOCK_SIZE - 1, BLOCK_SIZE - 1, YELLOW); //YELLOW
    }

    // Draw S-shape
  int s_start[4][2] = { { 0 + MIDDLE_OFFSET, 0 }, { 1 + MIDDLE_OFFSET, 0 }, { 1 + MIDDLE_OFFSET, 1 }, { 2 + MIDDLE_OFFSET, 1 } };
  for (int i = 0; i < 4; i++) {
      tft.fillRect(screenWidth / 2.5 + (s_start[i][0] * BLOCK_SIZE), screenHeight / 2.3 + (s_start[i][1] * BLOCK_SIZE), BLOCK_SIZE - 1, BLOCK_SIZE - 1, MAGENTA); //MAGENTA
    }

     // Draw I-shape
  int i_start[4][2] = { { 0 + MIDDLE_OFFSET, 0 }, { 1 + MIDDLE_OFFSET, 0 }, { 2 + MIDDLE_OFFSET, 0 }, { 3 + MIDDLE_OFFSET, 0 } };
  for (int i = 0; i < 4; i++) {
      tft.fillRect(screenWidth / 1.8 + (i_start[i][0] * BLOCK_SIZE), ((screenHeight / 2.3) + BLOCK_SIZE) + (i_start[i][1] * BLOCK_SIZE), BLOCK_SIZE - 1, BLOCK_SIZE - 1, GREEN); //GREEN
    }
  int JoyStickY = map(analogRead(JOYSTICK_Y_PIN), 0, 1023, 10000, -10000);
  int JoyStickX = map(analogRead(JOYSTICK_X_PIN), 0, 1023, -10000, 10000);
 
  // while (digitalRead(JOYSTICK_BUTTON_PIN) != 1) {
    while (!(analogRead(JOYSTICK_Y_PIN) > 900) ) { //|| !(analogRead(JOYSTICK_Y_PIN) == 0) || !(analogRead(JOYSTICK_X_PIN) > 900) || !(analogRead(JOYSTICK_X_PIN) == 0)
    delay(100);
  } 
